...
let machineWidth, machineHeight, machineTop, machineTopHeight, machineBottomHeight, machineBottomTop, maxArmLength
...
class WorldObject {
  ...
  resizeShadow() {
    elements.value.box.style.setProperty('--scale', 0.5 + this.h / maxArmLength / 2)
  }
...
class Toy extends WorldObject {
  constructor(props) {
    ...
    elements.value.box.append(this.el)
    const toy = this

    this.el.addEventListener('click', () => this.collectToy(toy))
    elements.value.toys.push(this)
  }
  collectToy(toy) {
    ...
    elements.value.clawMachine.classList.add('show-overlay')
    settings.value.collectedNumber++
    elements.value.collectionBox.appendChild(
      Object.assign(document.createElement('div'), {
        className: `toy-wrapper ${settings.value.collectedNumber > 6 ? 'squeeze-in' : ''
          }`,
        innerHTML: `<div class="toy pix ${toy.toyType}"></div>`,
      }),
    )
    setTimeout(() => {
      elements.value.clawMachine.classList.remove('show-overlay')
      if (!document.querySelector('.selected'))
        elements.value.collectionArrow.classList.remove('active')
    }, 1000)
  }
  setRotateAngle() {
    const angle =
...
onMounted(() => {
  elements.value.clawMachine = document.querySelector('.claw-machine')
  elements.value.box = document.querySelector('.box')
  elements.value.collectionBox = document.querySelector('.collection-box')
  elements.value.collectionArrow = document.querySelector('.collection-arrow')

  const {
    width: mw,
    height: mh,
    top: mt,
  } = elements.value.clawMachine.getBoundingClientRect()

  machineWidth = mw
  machineHeight = mh
  machineTop = mt

  const { height: mth } = document
    .querySelector('.machine-top')
    .getBoundingClientRect()

  machineTopHeight = mth

  const { height: mbh, top: mbt } = document
    .querySelector('.machine-bottom')
    .getBoundingClientRect()

  machineBottomHeight = mbh
  machineBottomTop = mbt

  maxArmLength = machineBottomTop - machineTop - machineBuffer.y

  elements.value.box.style.setProperty('--shadow-pos', `${maxArmLength}px`)
  ...
  const getClosestToy = () => {
    ...
    const overlappedToys = elements.value.toys.filter(t => {
  ...
  const dropToy = () => {
    ...
    if (settings.value.targetToy) {
      settings.value.targetToy.z = 3
      settings.value.targetToy.move({
        moveKey: 'y',
        target: machineHeight - settings.value.targetToy.h - 30,
        moveTime: 50,
      })
    ...
    setTimeout(() => {
      ...
      activateHoriBtn()
      if (settings.value.targetToy) {
        settings.value.targetToy.el.classList.add('selected')
        elements.value.collectionArrow.classList.add('active')
        settings.value.targetToy = null
      }
    }, 700)
  }
  ...
  const grabToy = () => {
    if (settings.value.targetToy) {
      ;[vertRail, armJoint, arm].forEach(
        obj => (obj.moveWith[0] = settings.value.targetToy),
      )
      settings.value.targetToy.setRotateAngle()
      settings.value.targetToy.el.classList.add('grabbed')
    } else {
